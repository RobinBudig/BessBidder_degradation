...
RESUME_FROM_DATE_2024 = date(2024, 9, 7)  # ab diesem Datum neu starten

def file_start_date_from_name(filename: str) -> Optional[date]:
    """
    Erwartet z.B. 'Continuous_Trades-DE-20240907-20240908T001023000Z.csv'
    und gibt date(2024, 9, 7) zurück.
    """
    try:
        parts = filename.split("-")
        # parts = ["Continuous_Trades", "DE", "20240907", "20240908T001023000Z.csv"]
        start_str = parts[2]  # "20240907"
...
def execute_etl_transactions_new_format(years: List[int]) -> None:
    ...
    for year in years:
        with tempfile.TemporaryDirectory(dir=tempfile.gettempdir()) as tmpdirname:
            ...
            # Sortieren (macht den Ablauf reproduzierbarer)
            filenames = sorted(filenames)
            logging.info(f"[{year}] Found {len(filenames)} CSV files in {unpacked_archive_path}")

            for idx, filename in enumerate(filenames):

                # >>> NEU: Skip-Logik für 2024 <<<
                if year == 2024:
                    start_date = file_start_date_from_name(filename)
                    if start_date is not None and start_date < RESUME_FROM_DATE_2024:
                        logging.info(
                            f"Skipping {filename} (start_date={start_date}) "
                            f"because it is before resume date {RESUME_FROM_DATE_2024}"
                        )
                        continue
                # <<< ENDE NEU >>>
